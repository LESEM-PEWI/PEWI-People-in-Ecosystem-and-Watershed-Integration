...
function drawD3EconProfitPieChart(year, isTheChartInCategoryMode) {
  // RESETTING THE TEMPORARY COLOR AND LEGEND ELEMENT nameArray
  tempLegendItems = [];
  tempLegendColors = [];
  //remove the html that's already there, ie clear the chart
  document.getElementById('resultsFrame').contentWindow.document.getElementById('econProfitPieChart').innerHTML = " ";
  //pass data to the page that it needs, we do this by putting it in hidden divs
  document.getElementById('resultsFrame').contentWindow.document.getElementById('landYear').innerHTML = year;
  document.getElementById('resultsFrame').contentWindow.document.getElementById('upTo').innerHTML = boardData[currentBoard].calculatedToYear;
  ...
  */
  ...
  //document.getElementById('resultsFrame').contentWindow.document.getElementById('chart').innerHTML = "" ;
  var chart = document.getElementById('resultsFrame').contentWindow.document.getElementById('econProfitPieChart');
//d3 stuff here, I won't comment this section too heavily as it is mostly typical graphics
  var svg = d3.select(chart)
      .append('svg')
      .attr("class", "graph-svg-component")
...
// TODO please check these repetitions
    ...
    backendDataIdentifiers = [
      "C02_e", 'CO2_emissions', "CH4", "N2O",
      "SOC", "grossErosion",
      "aquaticHealth", "nitrateConcentration","phosphorusLoad", "sedimentDelivery",
      "gameWildlifePoints", "biodiversityPoints", "streamBiodiversity", "musselPopulation",
    ];
...
// TODO please check these repetitions
    ...
    for (var l = 0; l < backendDataIdentifiers.length; l++) {
      ...
      switch (l) {
        case 0:
          htmlTableString += "<tr class='tableHeading'><td class='verticalLine'><b>Climate</b></td>" + format + "</tr>"
          break;
        case 4:
          htmlTableString += "<tr class='tableHeading'><td class='verticalLine'><b>Soil</b></td>" + format + "</tr>"
          break;
        case 6:
          htmlTableString += "<tr class='tableHeading'><td class='verticalLine'><b>Water</b></td>" + format + "</tr>"
          break;
        case 10:
          htmlTableString += "<tr class='tableHeading'><td class='verticalLine'><b>Habitat</b></td>" + format + "</tr>"
          break;
      } //end switch
      // Column 1
      htmlTableString += "<tr>";
      ...
      for (var y = 1; y <= upToYear; y++) {
        ...
        if(backendDataIdentifiers[l] === "streamBiodiversity") {
          ...
          htmlTableString += Math.round(economics.GHGsScore[y][0]?.SOC, 1)+ "<br>";
        }
        else if (backendDataIdentifiers[l] === 'CO2_emissions') {
          htmlTableString += economics.GHGsScore[y][0]?.CO2_emissions + "<br>";        }
        else {
      ...
      for (var y = 1; y <= upToYear; y++) {
        ...
        if(backendDataIdentifiers[l] === "streamBiodiversity") {
          ...
          htmlTableString += economics.GHGs[y][0]?.CO2_emissions + "<br>";        }
        else {
          htmlTableString += addCommas((Math.round(Totals?.[tempString]?.[y] * 10) / 10).toFixed(1)) + "<br>";
        }
        htmlTableString += "</td>";
      ...
      if (backendDataIdentifiers[l] === "musselPopulation") {
        ...
        htmlTableString += "<td class='verticalLine centerText'>tons</td>";
      }
      ...
      else if (backendDataIdentifiers[l] === "biodiversityPoints") {
        htmlTableString += "<td class='verticalLine centerText'>pts</td>";
      }
      else if (backendDataIdentifiers[l] === "streamBiodiversity") {
    ...
    else if (l < 2) htmlTableString += "<td class='verticalLine centerText'>tons</td>";
      ...
      // Column 4
      else htmlTableString += "<td class='verticalLine centerText`>";
      for (var y = 1; y <= upToYear; y++) {
        htmlTableString += "<td  class='rightText'>";
        var tempString = backendDataIdentifiers[l];
        if(backendDataIdentifiers[l] === "streamBiodiversity") {
          ...
          htmlTableString += addCommas((Math.round(Totals.gameWildlifePoints[y] * 10) /10).toFixed(1)) + "<br>";
        }
        else if (backendDataIdentifiers[l] === "biodiversityPoints"){
          htmlTableString += addCommas((Math.round(Totals.biodiversityPoints[y] * 10) /10).toFixed(1)) + "<br>";
        }
        else if (backendDataIdentifiers[l] === "CH4") {
          ...
          htmlTableString += economics.GHGs[y][0]?.CO2_emissions + "<br>";  + "<br>";
        }  
        else if (backendDataIdentifiers[l] === "carbonSequestration") {
      ...
      //units cell
      if (backendDataIdentifiers[l] === "musselPopulation") {
        htmlTableString += "<td class='centerText'></td>";
      }
      else if (backendDataIdentifiers[l] === "streamBiodiversity") {
        htmlTableString += "<td class='centerText'>pts</td>";
      }
      ...
      else if (l < 2) htmlTableString += "<td class='centerText'>Mg</td>";
      else if (2 <= l && l < 4) htmlTableString += "<td class='centerText'>Mg</td>";
      else if (4 <= l && l < 5) htmlTableString += "<td class='centerText'>Mg</td>";
    ...
    for (var l = 0; l < backendDataIdentifiers.length; l++) {
      ...
      switch (l) {
        case 0:
          ...
          htmlTableString += "<tr class='tableHeading'><td class='verticalLine'><b>Yield</b></td>" + format + "</tr>"
      ...
      for (var y = 1; y <= upToYear; y++) {
        ...
        let cornYieldMax = boardData[currentBoard].maximums.cornMax;
        let soyYieldMax = boardData[currentBoard].maximums.soybeanMax;
        let cornYieldAdjScore = (bmpBudgetYields[y].cornYield / cornYieldMax) * 100;
        let soyYieldAdjScore = (bmpBudgetYields[y].soybeanYield / soyYieldMax) * 100;
        htmlTableString += "<td class='rightText'>";
        var tempString = backendDataIdentifiers[l] + "Score";
        //TODO
...
function render(years){
  ...
  */
  ...
  // To generate random precipitation values
  function generateRandomPrecipitation() {
    ...
    randomPrecipContainer.innerHTML = '';
...
function horizontalBarGraph(year){
  ...
  svg.append("text")
      .attr("x", width / 2)
      .attr("y", height + margin.bottom - 5) // Adjust this value to shift lower/higher
      .attr("text-anchor", "middle")
...
// Revenue Value Labels
  ...
  // Legend Positioning
  var colors = ["#8cc751", "#006400"]; // Cost (Red), Revenue (Blue)
